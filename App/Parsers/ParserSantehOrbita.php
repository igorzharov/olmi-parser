...
class ParserSantehOrbita extends Parser
{
    ...
    private function parserProducts()
    {
        ...
        foreach ($relations as $relation)
        {
            ...
            $insertColumns = ['product_id' => $relation['product_id'], 'title' => $title, 'description' => $description, 'price' => $price, 'image' => $image, 'product_url' => $relation['product_url'], 'date_added' => $dateNow, 'date_modify' => $dateNow, 'parser_class' => $this->parserClassName, 'is_parsed' => 0, 'is_update' => 0, 'status' => $status];
